...
then
    ...
    cleanup $BASEFOLDER
    # docker
    DOCKER="docker exec -i main-mysql"
    ...
    databases=$($DOCKER mysql -h$DBHOST -u"$DBUSER" -p"$DBPASSWORD" -P"$DBPORT" -e "SHOW DATABASES;" | tr -d "| " | grep -v Database)
    for db in $databases; do
        # Check if the database is in the skip list
        if [[ ",$SKIP_DB," != *",$db,"* ]]; then
            echo "| Dumping database: $db"
            $DOCKER mysqldump -h"$DBHOST" -u"$DBUSER" -p"$DBPASSWORD" -P"$DBPORT" $db > $SQLFOLDERFULL/$THEDATE.$db.sql
        else
